...
class Dialog
{
	...
	static async Notify({title='Alert', text='Please pay attention', showFor=3000}={})
	{
		var me = this;
		var promise = new Promise( (resolve, reject)=>
			{
				this.dialogBox = document.createElement( "dialog");
				this.dialogBox.innerHTML = '<h2 id="dialogTitle">' + title + '</h2><div id="dialogText">' + text + '</div>';
				this.dialogBox.id="confirmDialog";
				this.result = null;
				document.body.appendChild( this.dialogBox);
                setTimeout( () => { Dialog.#pushedYes( resolve);}, showFor);
				this.dialogBox.showModal();
			});
