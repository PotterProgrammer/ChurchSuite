...
sub scheduleReminder($$$)
{
	...
	if ( $^O =~ /Win/)
	{
		...
		my $cmd = sprintf( "schtasks /create /f /sc WEEKLY /d %s  /ST %02d:%02d /tn \"SendSchedulerReminders\" /tr \"" .
			               "curl -b UID=%s http://localhost:%d/scheduler/sendReminders\"", $days{$weekday}, $hour, $minute, getAdminUID(), $port);
	...
	{
		my $newLine = sprintf( "%02d %02d * * %02d curl -b UID=%s http://localhost:%d/scheduler/sendReminders", $minute, $hour, $weekdays{$weekday}, getAdminUID(), $port);
